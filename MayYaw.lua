...
local MainYaw=gui.Groupbox(MayYaw, "MayYaw", 5, 10, 175, 0)
local EnableYaw=gui.Checkbox(MainYaw, "Enableyaw", "开启", 1)
local ComboboxMenuMode=gui.Combobox(MainYaw, "ComboxMenuMode", "选择功能菜单","Anti-Aim 反自瞄","视觉","其他")
local GroupboxMain=gui.Groupbox(MayYaw, "请先开启总开关", 190, 10, 410, 0)
local GroupboxAntiAim=gui.Groupbox(MayYaw, "MayYaw Anti-Aim 反自瞄", 190, 10, 410, 0)
local GroupboxVisuals=gui.Groupbox(MayYaw, "MayYaw 视觉", 190, 10, 410, 0)
local EnableIndicators=gui.Checkbox(GroupboxVisuals, "EnableIndocators", "指示器", 0)
local EnableKeybinds=gui.Checkbox(GroupboxVisuals, "EnableKeybinds", "按键列表", 0)
local EnableDesyncInvertIndicator=gui.Checkbox(GroupboxVisuals, "EnableDesyncInvertIndicator", "假身指示器", 0)
local EnableWatermark=gui.Checkbox(GroupboxVisuals,"EnableWatermark","水印",0)
local EnableClantag=gui.Checkbox(GroupboxVisuals,"EnableClantag","组名",0)
local AspectRatioDefValSlider=gui.Slider(GroupboxVisuals,"AspectRatioVal","屏幕拉伸",0,0,200)
local GroupboxMisc=gui.Groupbox(MayYaw, "MayYaw 其他选项", 190, 10, 190, 0)
local GroupboxAutoBuy=gui.Groupbox(MayYaw, "自动购买", 190, 250, 190, 0)
local ComboboxAutoBuyPrimaryWeapon=gui.Combobox(GroupboxAutoBuy, "ComboxAutoBuyPrimaryWeapon", "主武器","无","连狙","鸟狙","AWP")
local ComboboxAutoBuySecondaryWeapon=gui.Combobox(GroupboxAutoBuy, "ComboxAutoBuySecondaryWeapon", "副武器","无","沙鹰/R8左轮","双枪","Tec9/FN57")
local ComboboxAutoBuyArmor=gui.Combobox(GroupboxAutoBuy, "ComboxAutoBuyArmor", "护甲","无","防弹衣","防弹衣 + 头盔")
local GrenadeMultibox=gui.Multibox(GroupboxAutoBuy,"投掷物")
local EnableBuyGrenade=gui.Checkbox(GrenadeMultibox, "EnableBuyGrenade", "手雷", 0)
local EnableBuyMolotov=gui.Checkbox(GrenadeMultibox, "EnableBuyMolotov", "燃烧弹", 0)
local EnableBuySmoke=gui.Checkbox(GrenadeMultibox, "EnableBuySmoke", "烟雾弹", 0)
local EnableDmg=gui.Checkbox(GroupboxMisc,"EnableDmg","最小伤害覆盖",0)
local EnableEngineRadar=gui.Checkbox(GroupboxMisc,"EnableEngineRadar","小地图红点透视",0)
local EnableJumpScoutFix=gui.Checkbox(GroupboxMisc,"EnableJumpScoutFix","跳狙增强",0)
local EnableAutoBuy=gui.Checkbox(GroupboxMisc,"EnableAutoBuy","自动购买",0)
local EnableHitLog=gui.Checkbox(GroupboxMisc,"EnableHitLog","高级伤害记录",0)
local GroupboxDMG=gui.Groupbox(MayYaw, "最小伤害覆盖", 430, 10, 200, 0)
local AwpDMGOverrideSlider=gui.Slider(GroupboxDMG, "awpdmgoverrideslider", "AWP 最小伤害覆盖", 0, 1, 130 )
local AutoDMGOverrideSlider=gui.Slider(GroupboxDMG, "autodmgoverrideslider", "连狙 最小伤害覆盖", 0, 1, 130 )
local Ssg08DMGOverrideSlider=gui.Slider(GroupboxDMG, "ssg08dmgoverrideslider", "鸟狙 最小伤害覆盖", 0, 1, 130 )
local HeavyPistolDMGOverrideSlider=gui.Slider(GroupboxDMG, "heavypistoldmgoverrideslider", "沙鹰/R8左轮 最小伤害覆盖", 0, 1, 130 )
local PistolDMGOverrideSlider=gui.Slider(GroupboxDMG, "pistoldmgoverrideslider", "轻手枪 最小伤害覆盖", 0, 1, 130 )
local ComboboxDMGmode=gui.Combobox(GroupboxDMG, "ComboboxDMGmode", "模式","长按","开关切换")
local DMGKey=gui.Keybox(GroupboxDMG,"DMGKey","按键", 0 )
local EnableMayYawAA=gui.Checkbox(GroupboxAntiAim,"EnableMayYawAA","MayYawAA 反自瞄",0)
local EnableCustomMayYawAA=gui.Checkbox(GroupboxAntiAim,"EbableCustomMayYawAA","自定义 MayYawAA",0)
local EnableLagitAAonUse=gui.Checkbox(GroupboxAntiAim,"EnableLagitAAonUse","按E切换合法AA",0)
local EnableAdvancedAtTarget=gui.Checkbox(GroupboxAntiAim,"EnableAdvancedAtTarget", "高级目标侦测",0)
local ComboxAtTargetPriotity=gui.Combobox( GroupboxAntiAim, "ComboxAtTargetPriotity", "目标侦测优先级", "根据准心方向", "根据距离远近" )
local GroupboxCustomMayYawAA=gui.Groupbox(MayYaw, "自定义 MayYaw AA",  190, 268, 410, 0)
local RotationSliderCustom=gui.Slider(GroupboxCustomMayYawAA, "RotationSliderCustom", "旋转角度偏移", 0, -58, 58 )
local LBYSliderCustom=gui.Slider(GroupboxCustomMayYawAA, "LBYSliderCustom", "LBY 偏移", 0, -180, 180 )
local BaseYawSliderCustom=gui.Slider(GroupboxCustomMayYawAA,"BaseYawSliderCustom","基础角度偏移",0,-180,180)
local EnableLowDelta=gui.Checkbox(GroupboxCustomMayYawAA, "EnableLowDelta", "小角度偏移",0)
local LowDeltaSliderValue=gui.Slider(GroupboxCustomMayYawAA, "LowDeltaSliderValue", "小角度偏移数值", 0, 1, 60 )
local EnbaleAutoSwitchDesync=gui.Checkbox(GroupboxCustomMayYawAA,"EnbaleAutoSwitchDesync","自动假身切换",0)
local ComboboxAutoDesyncInvertMode=gui.Combobox(GroupboxCustomMayYawAA, "ComboboxAutoDesyncInvertMode", "假身切换模式","根据准心方向切换","根据距离远近切换","根据自身移动速度切换")
local DesyncSwitchKey=gui.Keybox(GroupboxCustomMayYawAA,"DesyncSwitchKey","手动假身切换", 0 )
...
local WatermarkColor=gui.ColorPicker(EnableWatermark,"Colorwatermark","水印颜色", 56,56, 165, 255 )
local KeybindsColor=gui.ColorPicker(EnableKeybinds,"Colorwatermark","按键列表颜色", 56,56, 165, 255 )
local MainLogColor=gui.ColorPicker(EnableHitLog,"MainLogColor","主事件记录颜色", 94,152,217, 255 )
local PrefixLogColor=gui.ColorPicker(EnableHitLog,"PrefixLogColor","记录行首颜色", 0,243,26, 255 )
local DesyncInvertActiveColor=gui.ColorPicker(EnableDesyncInvertIndicator,"DesyncInvertActiveColor","方向箭头激活颜色", 0,255, 0, 255 )
--[All ffi
ffi.cdef [[
